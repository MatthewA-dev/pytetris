  ...
  def getmove(self,piece):
    ...
    # get the furthest to the left coord that's valid for every piece rotation
      ...
      while not self.board.checkOut(piece, pc):
        ...
        while not self.board.checkOverlapping(piece,piececoords) and not self.board.checkInGround(piece, piececoords):
          piececoords = (piececoords[0], piececoords[1] + 1)
        piececoords = (piececoords[0], piececoords[1] - 1)

        score = self.evaluate(self.board.placePiece(True, piece, piececoords))
...
class Board():
  ...
  def shiftPiece(self, xshift):
    ...
    if(self.checkOverlapping(self.piece, self.piececoords)):
      self.piececoords = (self.piececoords[0] - xshift, self.piececoords[1])
  ...
  def rotatePiece(self, rotdir, piece, returnPiece):
    ...
    if(not returnPiece):
      if(self.checkOverlapping(rotpiece, self.piececoords) or self.checkOut(rotpiece, self.piececoords)):
  ...
  def fitPiece(self, piece):
    ...
    for xchange in range(MAXCHANGE*2 + 1):
      ...
      for ychange in range(MAXCHANGE*2 + 1):
        ...
        if(self.checkOverlapping(piece,tempcoords) or self.checkOut(piece, tempcoords)):
          continue
        else:
  ...
  def checkOverlapping(self, piece, piececoords):
    for yindex, piecerow in enumerate(piece):
      for xindex, p in enumerate(piecerow):
        try:
          if(self.grid[piececoords[1] + yindex][piececoords[0] + xindex] != 0 and p != 0):
            return True
        except IndexError:
          pass
    return False
  ...
  def render(self):
    self.surface.fill((0,0,0))
        # REMOVE THIS
    score = pygame.font.Font('Minecraftia.ttf', 30).render("Grade: " + str(AI.evaluate(AI, self.grid)), True, (255,255,255))
      ...
      if event.type == pygame.QUIT:
        ...
        if(not hasAI):
          ...
          if pygame.key.get_pressed()[pygame.K_DOWN]:
            #HARDDROP
            while not board.checkOverlapping(board.piece,board.piececoords) and not board.checkInGround(board.piece, board.piececoords):
